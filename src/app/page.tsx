...
const freelance: Entry[] = [
  ...
  },
  {
    ...
    bulletsKo: [
      ...
      "Hub-Spoke 12 에이전트(Engineering 9 + Finance 3) — CTO 라우터 → PO · Planner → Dual Squad(Squad 1: attendance · organization / Squad 2: reporting · communication) Lead · Dev · QA. 16 Skill + 24 event handler + 9 Slack intent command 운용 중.",
      "21-Crack pipeline guardrails(C1–C21)이 \"YAML은 LLM에 대한 권고일 뿐\"이라는 전제 위에서 phase skip · scorecard 누락 · anti-deception을 이벤트 단위로 검출하고 자동 되돌립니다. 운영 안전선은 Per-agent + company-wide circuit breaker로 묶입니다.",
      "자매 코드베이스 kiki-appmaker는 컨셉이 다릅니다. Kiki가 레거시 유지보수 오케스트레이션이라면, AppMaker는 사용자 한 줄 프롬프트를 받아 PM → CDO → CTO → QA Lead가 PDCA 사이클을 돌리며 \"공장형으로 앱을 찍어내는\" 파이프라인입니다. 17-agent external-company provisioning + 5단계 install workspace + PIN match-3 데모.",
    ],
    chips: [
      ...
      "Dual squad",
      "C1–C21 guardrails",
      "Sister: kiki-appmaker (factory pipeline)",
    ],
    accent: "var(--acc-line)",
  },
];
