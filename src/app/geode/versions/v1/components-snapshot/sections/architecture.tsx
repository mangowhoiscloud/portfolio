...
const layers = [
  ...
  },
  {
    id: "L4",
    name: "Extensibility",
    detail: "47 Tools + 44 MCP · PolicyChain · 20 Skills",
    color: "#F5C542",
          ...
          >
            {layers.map((layer) => (
              ...
              >
                ...
                {/* Name */}
                <div className="font-medium text-sm text-white/200 w-[110px] md:w-[130px] shrink-0">
                  {layer.name}
                </div>

                {/* Detail — hidden on mobile */}
                <div className="text-xs text-[#5A6A8A] hidden md:block flex-1 pr-12">
                  {layer.detail}
                </div>
        ...
        <ScrollReveal delay={0.5}>
          <div className="mt-6 flex items-center gap-3 px-4 py-3 rounded-xl border border-dashed border-[#4ECDC4]/15 bg-[#4ECDC4]/[0.02]">
            ...
            <div>
              ...
              <span className="text-sm text-[#5A6A8A] ml-2">
                45 events · 우선순위 기반 · 비차단 실행 — L0~L5 전 레이어를 수직 관통하는 교차 관심사
              </span>
            </div>
          </div>
        </ScrollReveal>
