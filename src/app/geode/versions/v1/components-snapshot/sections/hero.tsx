      ...
      <div className="relative z-10 flex flex-col items-center gap-5 px-6">
        ...
        >
          ...
          <p className="text-sm text-white/35 mt-1">
            Scaffold가 만들고, Loop가 실행하고, Runtime이 검증하는 자율 실행 하네스
          </p>
        </motion.div>
        ...
        >
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              ...
              <div className="text-[10px] font-mono text-[#5A6A8A] uppercase tracking-widest mt-1">
                {s.label}
              </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="flex items-center gap-3 mt-4">
          <Link
            ...
            target="_blank"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-white/[0.06] text-xs font-mono text-[#5A6A8A] hover:text-[#8B9CC0] hover:border-white/[0.12] transition-all duration-300"
          >
