        ...
        <ScrollReveal delay={0.08}>
          <div className="overflow-x-auto -mx-4 px-4 pb-2 mb-8">
            <svg viewBox="0 0 760 170" className="w-full min-w-[560px]" style={{ maxHeight: 200 }}>
              ...
              <text x={380} y={22} textAnchor="middle" fill="white" fillOpacity={0.18} fontSize={9} fontFamily="ui-monospace, monospace" letterSpacing="0.1em">INBOUND → ROUTE → SESSION → LOOP → RESPOND</text>
            </svg>
          </div>
        </ScrollReveal>
            ...
            <div className="space-y-2 mb-4">
              {mode.flow.map((f, i) => (
                <div key={f.step} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-[#5A6A8A]">{f.detail}</span>
                </div>
              ))}
            </div>
