...
const phases = [
  ...
  },
  {
    ...
    desc: "Scaffold(CLAUDE.md + CI) → Runtime(58 Hooks + 41 Skills) → Loop(while True + 8 Guards) 3중 구조 완성. Slack Gateway, SubAgent DAG, 5-Tier Memory, LLM Resilience.",
    descEn: "Completed 3-layer structure: Scaffold (CLAUDE.md + CI) → Runtime (58 Hooks + 41 Skills) → Loop (while True + 8 Guards). Slack Gateway, SubAgent DAG, 5-Tier Memory, LLM Resilience.",
    badge: "CORE",
    badgeEn: "CORE",
    badgeColor: "#4ECDC4",
    versions: ["v0.24 Harness", "v0.28 Gateway", "v0.31 GitFlow", "v0.37 4-Layer", "v0.48 Hook Interceptor"],
  },
  {
    phase: "Fork",
    ...
    desc: "산업 적용 전 DomainPort 교체 가능성 실증. Java 1.8→22 + Spring 4.3→6.1 자동 마이그레이션. 5,523파일 엔터프라이즈 프로젝트 83/83 테스트 통과. 에러 4분류 라우팅(CONFIG/CODE/BEHAVIOR/ENV) + Architect/Editor 분리. 5시간 48분, 1,133턴 자율 수행, $388.",
    descEn: "Validated DomainPort swappability before industrial deployment. Java 1.8→22 + Spring 4.3→6.1 automated migration. 83/83 tests passing on a 5,523-file enterprise project. 4-class error routing (CONFIG/CODE/BEHAVIOR/ENV) + Architect/Editor separation. 5h 48m, 1,133 autonomous rounds, $388.",
    badge: "FORK",
    badgeEn: "FORK",
    badgeColor: "#C084FC",
    versions: ["v0.21", "83/83 tests", "$388 / 1,133 rounds / 5h48m"],
  },
  {
    phase: "Phase 4",
    period: "2026.04 ~ 04.28",
    title: "R1~R9. Reasoning Wire Audit",
    titleEn: "R1~R9. Reasoning Wire Audit",
    desc: "9-cycle reasoning depth audit. 4 프로바이더의 reasoning wire kwargs 정렬 (Anthropic xhigh, OpenAI Responses encrypted_content + summary, Codex Plus replay walker, GLM thinking field). R6 IPC surfacing. R9 라이브 와이어 테스트.",
    descEn: "9-cycle reasoning depth audit. Aligned reasoning wire kwargs across 4 providers (Anthropic xhigh, OpenAI Responses encrypted_content + summary, Codex Plus replay walker, GLM thinking field). R6 IPC surfacing. R9 live wire tests.",
    badge: "R-SERIES",
    badgeEn: "R-SERIES",
    badgeColor: "#F4B8C8",
    versions: ["v0.55 R1 Codex replay", "v0.56 R4 xhigh", "v0.57 R6 IPC", "v0.58 R2 GLM thinking", "v0.60 R3 PAYG", "v0.62 R9 live"],
  },
  {
    phase: "Phase 5",
    period: "2026.04.29 ~ 05.02",
    title: "Lifecycle + Plugin Split + Cache",
    titleEn: "Lifecycle + Plugin Split + Cache",
    desc: "Lifecycle commands (/stop /clean /uninstall /status). Game IP 도메인이 plugins/game_ip/ 로 분리 — \"domain is a plugin\" 테제의 구조적 증거. Anthropic messages cache_control (Hermes system_and_3 parity).",
    descEn: "Lifecycle commands (/stop /clean /uninstall /status). Game IP domain extracted to plugins/game_ip/ — structural evidence for the \"domain is a plugin\" thesis. Anthropic messages cache_control (Hermes system_and_3 parity).",
    badge: "CURRENT",
    badgeEn: "CURRENT",
    badgeColor: "#4ECDC4",
    versions: ["v0.63 Lifecycle", "v0.64 Plugin Split", "v0.65 Messages Cache", "236 modules · 4380+ tests"],
  },
];
