          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-white/[0.08] bg-white/[0.02]">
                <th className="text-left p-3 font-mono uppercase tracking-wider text-[10px] text-white/45 w-[18%]">
                  {t(locale, "패턴", "Pattern")}
                </th>
                <th className="text-left p-3 font-mono uppercase tracking-wider text-[10px] text-[#4ECDC4]/70 w-[41%]">
                  {t(locale, "Artifact (출시되는 OS)", "Artifact (the OS that ships)")}
                </th>
                <th className="text-left p-3 font-mono uppercase tracking-wider text-[10px] text-[#F4B8C8]/70 w-[41%]">
            ...
            <tbody>
              {mirrors.map((row, i) => (
                <tr key={i} className="border-b border-white/[0.04] hover:bg-white/[0.015] transition-colors align-top">
                  <td className="p-3 font-mono text-[12px] text-white/55 italic">
                    {row.pattern}
                  </td>
                  <td className="p-3">
                    <div className="text-[#F0F0FF] font-medium mb-1">{row.artifactLabel}</div>
                    <div className="text-white/55 text-[12px] leading-relaxed mb-1.5">
                      {row.artifactBody}
                    </div>
                    <div className="font-mono text-[10.5px] text-[#FFD8A8]/70">
                      {row.artifactFile}
                    </div>
                  </td>
                  <td className="p-3">
                    <div className="text-[#F0F0FF] font-medium mb-1">{row.scaffoldLabel}</div>
                    <div className="text-white/55 text-[12px] leading-relaxed mb-1.5">
                      {row.scaffoldBody}
                    </div>
                    <div className="font-mono text-[10.5px] text-[#FFD8A8]/70">
